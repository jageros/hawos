...
def dump_to_mongo(name, data):
    ...
    conn = pymongo.MongoClient(MONGO_IP, MONG_PORT)
    db = conn[MONGO_DB]
    if MONGO_USER:
        db.authenticate(MONGO_USER, MONGO_PWD, mechanism='SCRAM-SHA-1')
    db.xlsxdata.update({"_id": name}, {"data":{"data":data}}, upsert=True)
